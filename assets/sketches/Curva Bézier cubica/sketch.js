...
function setup() {
  ...
  // Aggiungere quattro punti di controllo iniziali ridimensionati per adattarsi alla tela 500x300
  controlPoints.push(createVector(50, 250));   // P0
  controlPoints.push(createVector(150, 50));   // P1
  controlPoints.push(createVector(350, 50));   // P2
  controlPoints.push(createVector(450, 250));  // P3
}
...
function draw() {
  ...
  if (controlPoints.length === 4) {
    ...
    for (let i = 0; i < controlPoints.length; i++) {
      fill(0); // Tutti i punti di controllo sono neri
      ellipse(controlPoints[i].x, controlPoints[i].y, 10, 10);
      text(`P${i}`, controlPoints[i].x - 25, controlPoints[i].y - 15); // Mostra il tipo di punto
    }
    ...
    // Disegna la curva di Bézier cubica usando de Casteljau
    stroke(0);
    noFill();
    beginShape();
...
function keyPressed() {
  if (key === 'A' || key === 'a') {
    ...
    controlPoints.push(createVector(150, 50));
    controlPoints.push(createVector(350, 50));
